/*
	author dom schuette & vanessa schuler
	(C) HERE 2014
	*/
	...
	var gotRoutingResponse = function (respJsonObj)
	{
		...
		for(i = 0; i < shape.length; i++)
		{
			...
			path.push(new H.geo.Point(lat,lon));
		}
	...
	var findNearestSpeedLimit = function()
	{
		...
		for(i = 0; i < l; i++)
		{
			strip.pushLatLngAlt(path[i].lat,path[i].lng, null);
		}
		...
		var polyline = new H.map.Polyline(strip,
			...
			map.setCenter(objContainer.getBounds().getCenter());
			map.setZoom(14);
