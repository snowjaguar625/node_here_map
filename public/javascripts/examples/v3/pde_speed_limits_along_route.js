	...
	*/
	...
	var loadExample = function()
	{
		switch (document.getElementById("examples").value){
			...
			case "2": 
					document.getElementById("start").value = "Otsu";
					document.getElementById("dest").value = "Osaka";
	...
	********************************************************/
	...
	var go = function()
	{
		if(truck)
		{
			...
			layers["TRUCK_SPEED_LIMITS_FC"] = {callback: gotCONDSpeedLimits}; // be careful here, conditionalSpeeds stores truck speed limits as well.
	...
	var calculateRouteDirect = function(start, destination)
	{
		...
		var urlRoutingReq = "https://cle.api.here.com/2/calculateroute.json?waypoint0="+
	...
	function pdeManagerFinished(nrTilesRequested)
	{
		...
		for(;ii < ll; ii++)
		{
			...
			clink = conditionalSpeeds[linkID],  // be careful here, conditionalSpeeds stores truck speed limits as well
			dtp,
			variable = false;
					...
					if(clink[c].DATE_TIMES)
					{
						...
						if(speeds[linkID])
						{
							...
							if(routeLinkDir == '-' && speeds[linkID].SPEED_LIMIT != null){
								...
								speedLimit = getLowerSpeedLimit(clink[c].TO_REF_SPEED_LIMIT, speedLimit);
							}
								...
								speedLimit = getLowerSpeedLimit(clink[c].FROM_REF_SPEED_LIMIT, speedLimit);
							}
							else
			...
			if(!validCondition)
			{
				...
				var startPoint = [shape[0], shape[1]];
