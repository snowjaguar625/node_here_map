...
(function setValuesFromUrl() {
  ...
  */
  ...
  var cle = platform.ext.getCustomLocationService( document.getElementById('serverURL').value, app_id, app_code);
  ...
  var calculateRouteIsoline = (function () {
    ...
    return function (start, destination) {
     ...
     var poiReqParams = {
      ...
      var url = document.getElementById('serverURL').value + '/2/calculaterouteisoline.json?' + Object.keys(params).map(function (k) {
          return k + '=' + params[k]
      }).join('&');
  ...
  var searchResponse = function(data, arr, pos) {
   ...
   function onCheckPositionChanged( url, resp, err ) {
        ...
        if(resp.message !== null ) {
          feedbackTxt.innerHTML = 'Isoline POI Request received with an Error: ' + resp.message + '[' + resp.faultCode + ']';       
          return;
        }
