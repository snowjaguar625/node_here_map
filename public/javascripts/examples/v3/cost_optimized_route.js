/*
		...
		************************************/
		var calculateRoute = function (start, destination) {
			...
			//vspec += "&tollPass=transponder"; Why it should be always? It is already in input field as additionaly parameters
			...
			var appIdNeeded = true;
			if(finalParamsValue.indexOf("app_id=") != -1 || serverURL.value.indexOf("app_id=") != -1) {
				appIdNeeded = false;
			}
			...
			[
				...
				((serverURL.value.indexOf("tce") !== -1) ? ("&rollup=none,country;tollsys") : ("&rollups=none,country;tollsys")),
				...
				(appIdNeeded ? "&app_id=" + app_id : ""),
				(appIdNeeded ? "&app_code=" + app_code : ""),
		...
		function parseRoutingResponse(resp) {
			if (resp.error != undefined) {
				if (resp.error == "NoRouteFound") {
					...
					feedbackTxt.innerHTML = "<font color='red'>The Router service is unable to compute the route: try to change your start / destination point or change the vehicle profile</font>";
				}
				else {
					alert(resp.error);
					feedbackTxt.innerHTML = "<font color='red'>" + resp.error + "</font>";
				}
				return;
			}
			...
			if (resp.response == undefined && resp.issues !== undefined) {
				if(resp.issues.length > 0) {
					if (resp.issues[0].message.indexOf("Couldn't reach way point") != -1) {
						alert('Please consider to change your start or destination as the one you entered is not reachable with the given vehicle profile');
						feedbackTxt.innerHTML = "<font color='red'>The Router service is unable to compute the route: try to change your start / destination point or change the vehicle profile</font>";
					}
					else {
						alert(resp.issues[0].message);
						feedbackTxt.innerHTML = "<font color='red'>" + resp.issues[0].message + "</font>";
					}
				}
				return;
			}
				...
				for (var i = 0; i < routeTollItems.length; i++) {
					...
					for (var j = 0; j < routeTollItems[i].linkIds.length; j++) {
						...
						for(var li = 0; li < links.length; li++) {
							if(links[li].getStyle().strokeColor == routeColor[routeAlternative]) { // only change link color to toll color if not already modified
