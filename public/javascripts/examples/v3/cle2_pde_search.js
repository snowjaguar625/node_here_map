...
//  <!-- loadMRSVersionTxt(); -->
 ...
 function changeenv(that) {
   ...
   if(env==='cle'){
     document.getElementById('clefields').style.display = 'block'
           ...
           document.getElementById('environment').value = "https://cle.api.here.com";
   }
 ...
 function envSelect() {
   ...
   if(service === 'PDE'){
       ...
       cle.getPDELayers(onGetMapLayers); //reload the PDE layers because with pde.api.here.com url we get only PDE layers and some layers e.g., ARCHIVED_WEATHER layer does not appear if the request goes with cle.api.here.com url
   }
 }
 ...
 function callCle() {
   ...
   switch( document.getElementById( 'searchType' ).value ) {
     ...
     case 'corridor':
               ...
               searchGeometry = new H.map.Polyline( new H.geo.Strip( latLngAlts ), searchGeometryOptions );
       break;
