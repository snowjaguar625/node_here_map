/**
         ...
         */
         ...
         if( host !== null && host.length != 0 ) {
             document.getElementById('endpoint').value = document.getElementById('endpoint').value.replace("https://fleet.api.here.com" , host);
         }
                 ...
                 for (var k = 0; k < resp.response.route[r].leg.length; k++){
                         ...
                         for (var i in maneuvers) {
                             if(maneuvers[i].position !== undefined) { // maneuvers could have no geometry if set &maneuverAttributes=none
                                 ...
                                 var marker = new H.map.Marker(point, { icon: createWarningIconMarker(humanReadabletime(maneuvers[i].travelTime), instr, "guidance",language) });
                 ...
                 for(var k = 0; k<resp.response.route[r].leg.length; k++){
                     ...
                     for (var m = 0; m < resp.response.route[r].leg[k].link.length; m++) {
                         ...
                         if (time != previousTime) {
                             elapsedTimeGroup.addObject(new H.map.Marker(link.getBounds().getCenter(), {icon: createTimeIcon("#000000", time, 17)}));
                             previousTime = time;
                         }
