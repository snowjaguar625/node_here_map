/*
	...
	*/
	...
	var isavoidturns;
	var stravoidtruns;
	var toggleavoidturns = 0;
	var toggleselection = 0;
	...
	document.getElementById("thickpolyline").innerText = '';
	document.getElementById("thinpolyline").innerText = '';
	
	document.getElementById('avoidturns').addEventListener('change', function(){
	toggleselection++;
	if(toggleselection%2 == 1){
		document.getElementById("thickpolyline").innerText = 'THICK polyline(route) represenst the route with avoidTurns enabled.';
		document.getElementById("thinpolyline").innerText = 'THIN polyline(route) represenst the route with avoidTurns disabled.';
	}
	else{
		document.getElementById("thickpolyline").innerText = '';
		document.getElementById("thinpolyline").innerText = '';				
	...
	********************************************************/
	...
	var region =
	{
		"EU" :
		{
			...
			destCoordinate: new H.geo.Point(50.1324, 8.56352),
			center :
			{
				lat: 50.12224202692113, 
				lng: 8.623469649755862
			},
	...
	var MultiplePolyline = function (route) {
		...
		//	console.log(currentRoute);
			...
			// set constant color, better for demoing	
			var color=  "rgba(8,37,226,1)"; // "rgba("+(Math.floor((Math.random() * 255) + 1)) + ","+ (Math.floor((Math.random() * 255) + 1)) +","+ (Math.floor((Math.random() * 255) + 1)) +","+ 1 + ")";
			
			
			if(toggleavoidturns == 0) {
			...
			/*var fence = new H.map.Polygon(strip,
			{
				style:
				{
					lineColor: "red",
	...
	var calculateRoute = function(wPoints)
	{
		...
		var isavoidturns = document.getElementById("avoidturns");
		var stravoidtruns = isavoidturns.options[isavoidturns.selectedIndex].value;
		
		if(stravoidtruns == "enabled"){
			toggleavoidturns = 1;
				dragRoute[currentRoute].calculateRouteParams = {
				'app_code': app_code,
				'app_id': app_id,
		...
		var calculateRouteParams1 = dragRoute[currentRoute].calculateRouteParams;
			
		if(lWeight > 0)
		calculateRouteParams1.limitedWeight = lWeight;
		if(aWeight > 0)
		calculateRouteParams1.weightPerAxle = aWeight;
		if(h > 0)
		calculateRouteParams1.height = h;
		if(w > 0)
		calculateRouteParams1.width = w;
		if(l > 0)
		calculateRouteParams1.length = l;
			
		var  dummy = router.calculateRoute(calculateRouteParams1, onResult, onError);
		}		
