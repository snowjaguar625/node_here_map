	...
	*/
		...
		if(group.getObjects().length == 0)
		{
			...
			for(n = 0; n < arr.length; n++)
			{
				...
				if(bbox.containsPoint(curPoint))
				{
					...
					var url = ["https://weather.cc.api.here.com/weather/",
					"1.0/",
					"report.json?",
					"name=",
					arr[n].name,
	...
	function requestWeatherViaLatLon()
	{
		...
		for(var i in coords)
		{
			var coordinate = coords[i],
				url = ["https://weather.cc.api.here.com/weather/",
				"1.0/",
				"report.json?",
				"latitude=",
				coordinate.lat,
	...
	function getCenterCoordinates()
	{
		...
		$.each(boundingBoxes, function(i, v) {
			...
			strip.pushPoint(v.ne);
			strip.pushPoint(new H.geo.Point(v.sw.lat, v.ne.lng));
			strip.pushPoint(v.sw);
			strip.pushPoint(new H.geo.Point(v.ne.lat, v.sw.lng));
