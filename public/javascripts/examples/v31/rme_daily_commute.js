...
var submitTrace = function () {
    ...
    //TODO : replace to Apikey url when Cors is supported by the end point
    // if there is an app_id and App_code specified in the URL then use it, otherwise use the default
    var url = document.getElementById('rmeUrl').value + "&attributes=LINK_FCn(REF_NODE_NEIGHBOR_LINKS,NONREF_NODE_NEIGHBOR_LINKS)&legal=access,gate,oneway,thrutraf,turn&file=" + encodeURIComponent(content);
...
var gotRouteMatchResponse = function (respJsonObj) {
    ...
    for (var l = 0; l < routeLinks.length; l++) {
        ...
        metersToDriveBeforeCreatingAnotherRouteRelevantPoint -= routeLinks[l].linkLength;
...
function routeAlongMatchedPoints(fromIdx, toIdx) {
    ...
    var onResult = function(result) {
        var strip = new H.geo.LineString(), shape = result.response.route[0].shape, i, l = shape.length;
        for (var i = 0; i < l; i++) {
