/*
	...
	*/
	...
	var overviewControl = new H.ui.Overview(maptypes.vector.normal.map);
	...
	********************************************************/
			...
			for ( h = 0; h < numberCon; h++) {
				...
				for ( i = 0; i < routeConSections.length; i++) {
					...
					if (section.Dep.Transport.mode != 20) {
						...
						var stripJourney = new H.geo.LineString();
	...
	var connectionOnOff0 = function() {
		...
		if (showConnection0) {
			...
			map.getViewModel().setLookAtData({
				bounds: polylineLayerCon0.getBoundingBox()
			});
		} else{
	...
	var connectionOnOff1 = function() {
		...
		if (showConnection1) {
			...
			map.getViewModel().setLookAtData({
				bounds: polylineLayerCon1.getBoundingBox()
			});
	...
	var connectionOnOff2 = function() {
		...
		if (showConnection2) {
			...
			map.getViewModel().setLookAtData({
				bounds: polylineLayerCon2.getBoundingBox()
			});
		} else{
	...
	var connectionOnOff3 = function() {
		...
		if (showConnection3) {
			...
			connectionOperators.innerHTML = operator0;
			map.getViewModel().setLookAtData({
				bounds: polylineLayerCon3.getBoundingBox()
			});
		} else {
