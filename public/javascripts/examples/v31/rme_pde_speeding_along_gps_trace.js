// author wobecker 
    ...
    var	map = new H.Map(mapContainer, maptypes.vector.normal.map, { center: new H.geo.Point(52.11, 0.68),
        zoom: 5,
					...
					marker.addEventListener("pointerenter", function(e)
						{
							...
							if (!linkDataInfoBubble)
							{
								linkDataInfoBubble = new H.ui.InfoBubble(e.target.getGeometry(),{content: "<div>Speed Limit: " + e.target.$speed + " " + e.target.$unit + "</div>"});
								ui.addBubble(linkDataInfoBubble);	
							}
							else
							{
								linkDataInfoBubble.setPosition(e.target.getGeometry());
								linkDataInfoBubble.setContent("<div>Speed Limit: " + e.target.$speed + " " + e.target.$unit + "</div>");
							}
							linkDataInfoBubble.open();
					});
