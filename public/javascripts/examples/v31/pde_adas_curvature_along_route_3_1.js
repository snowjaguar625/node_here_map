/*
	...
	*/
	...
	map = new H.Map(mapContainer, defaultLayers.vector.normal.map,
		{
			center: {lat:47.574356, lng:10.408155},
			zoom: 7
		}
	);
	...
	startInfoBubble = null,
	endInfoBubble = null,
	// color of curve lines
	curvatureColor = "rgba(72,218,208, 0.7)",
	// color of speed limit changed
	speedLimitChangedColor = "rgba(15, 22, 33, 1)",
	// color of curve warnings
	speedWarningColor = "rgba(196,28,51, 0.7)",//
	...
	var colorExplanation = "<span style=\"font-weight: normal;color:" + rgb2hex(curvatureColor) + ";\">Green: Curvature</span>";
	...
	var currentRouteLineString = new H.geo.LineString();
	// truck icon
	...
	********************************************************/
	function handleLongClickInMap(currentEvent)
	{
		...
		if(bLongClickUseForStartPoint)
		{
			...
			if(startInfoBubble !=  null)
			{
				ui.removeBubble(startInfoBubble)
			}
			startInfoBubble = createGeocodeInfoBubble(line1, line2,pointA)
			ui.addBubble(startInfoBubble);
			removeXFromInfoBubble(startInfoBubble);
		...
		else
		{
			...
			if(endInfoBubble !=  null)
			{
				ui.removeBubble(endInfoBubble)
			}
			endInfoBubble = createGeocodeInfoBubble(line1, line2,pointB)
			ui.addBubble(endInfoBubble);
			removeXFromInfoBubble(endInfoBubble);
	...
	var gotRoutingResponse = function (respJsonRouteObj)
	{
		...
		if (respJsonRouteObj.type != undefined && respJsonRouteObj.type =="ApplicationError")
		{
			...
			for(var m = 0; m < respJsonRouteObj.response.route[0].leg[0].link.length; m++)
			{
				...
				for(i = 0; i < l; i++)
				{
					lineString.pushLatLngAlt.apply(lineString, shape[i].split(',').map(function(item) { return parseFloat(item); }));
					currentRouteLineString.pushLatLngAlt.apply(currentRouteLineString, shape[i].split(',').map(function(item) { return parseFloat(item); }));
				}

				var link = new H.map.Polyline(lineString,
					{
						style:
						{
							lineWidth: 5,
							strokeColor: "rgba(63,89,167, 0.4)",
							lineJoin: "round"
						}
					});
	...
	*/
	function renderCurvature(x, y, curvature, heading)
	{
		...
		// create polyline
		var lineString = new H.geo.LineString();
		lineString.pushLatLngAlt(lat, lon, 0);
		lineString.pushLatLngAlt(shiftedLatLon[0], shiftedLatLon[1], 0);

		var curvatureLine = new H.map.Polyline(lineString,
			{
				style:
				{
					lineWidth: 2,
	...
	*/
	...
	var createGeocodeInfoBubble = function(line1, line2, pos){
		var html ='';
		if(line2 == null)
			html = '<div><h7 style="white-space: nowrap">'+line1+'</h7></div>';
		else
			html = '<div><h6 style="white-space: nowrap">'+line2+'<br/>'+line1+'</h6></div>';
		return  new H.ui.InfoBubble(pos, { content: html });
	}

	//Hides the X from the info bubble - must be called after UI adds info bubble
	var removeXFromInfoBubble = function(infoBubble){
		infoBubble.getElement().getElementsByClassName("H_ib_close")[0].setAttribute("style","display:none;");
	}
	...
	{
		currentRouteLineString = new H.geo.LineString();
		listLinksOnRoute = [];
		routeLinkHashMap  = new Object();
		clearInfoBubble();
		clearGeocodedInfoBubble();
		stopRouteSimulation();
		iLastLinkSpeedlimit = 0;
		try
		{
	...
	// Helper for clearing the geocoded info bubbles
	function clearGeocodedInfoBubble()
	{
		if(startInfoBubble)
		{
			ui.removeBubble(startInfoBubble);
	...
	*/
	function handleUnitChange()
	{
		...
		speedLimitUnit = $('input[name=speedUnit]:checked').val(); 
	...
	*/
	...
	function startRouteSimulation()
	{
		...
		{
			// set route start
			var startCoord = currentRouteLineString.extractPoint(0);
	...
	var Walker = function (marker)
	{
		...
		this.walk = function ()
		{
			...
			marker.setGeometry(new H.geo.Point(coord[0], coord[1]));
