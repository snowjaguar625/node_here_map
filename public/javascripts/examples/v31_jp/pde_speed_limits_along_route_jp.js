...
*/
	...
	map.addEventListener('contextmenu', function (e) {
		...
		new H.util.ContextItem({
			label: 'Route From',
			callback: function() {
			document.getElementById('start').value = Math.abs(coord.lat.toFixed(6))+', '+Math.abs(coord.lng.toFixed(6));
			}
		}),
		...
		new H.util.ContextItem({
			label: 'Route To',
			callback: function() {
			document.getElementById('dest').value = Math.abs(coord.lat.toFixed(6))+', '+Math.abs(coord.lng.toFixed(6));
			}
		})
		);
	});
	...
	var loadExample = function()
	{
		switch (document.getElementById("examples").value){
			case "1":
					document.getElementById("start").value = "東京";
					document.getElementById("dest").value = "横浜市";
	...
	********************************************************/
	...
	var go = function()
	{
		...
		group = new H.map.Group();
		feedbackTxt.innerHTML = "Routing"; 
		//from: " + start.value + " to " + dest.value + " ...";
		geocodeVer7(start.value, true);
	};
	function validateGeoCode(input){
		inputAv7 = input.split(',');
		if(inputAv7.length ==2 ){
			ck_lat = /^(-?[1-8]?\d(?:\.\d{1,18})?|90(?:\.0{1,18})?)$/;
			ck_lon = /^(-?(?:1[0-7]|[1-9])?\d(?:\.\d{1,18})?|180(?:\.0{1,18})?)$/;
			var validLat = ck_lat.test(inputAv7[0].trim());
			var validLon = ck_lon.test(inputAv7[1].trim());
	...
	var geocodeVer7 = function(searchTerm, start){
		if(validateGeoCode(searchTerm)){
			if(start){
				pointA = new H.geo.Point(searchTerm.split(',')[0], searchTerm.split(',')[1]);
				geocodeVer7(dest.value, false);
			}
			else
			{
				pointB = new H.geo.Point(searchTerm.split(',')[0], searchTerm.split(',')[1]);
			...
			$.ajax({
				...
				start? pointA = new H.geo.Point(pos.lat, pos.lng) :pointB = new H.geo.Point(pos.lat, pos.lng);
	...
	var calculateRouteDirect = function(start, destination)
	{
		...
		var urlRoutingReq = "https://fleet.ls.hereapi.com/2/calculateroute.json?waypoint0="+
			...
			(truck ? "truck" : "car") + ";traffic:enabled&apikey=" + api_key_jp + "&jsoncallback=gotRoutingResponse";
	...
	function pdeManagerFinished(nrTilesRequested)
	{
		...
		feedbackTxt.innerHTML = "Done. Requested " + nrTilesRequested + " Advanced Datasets tiles for " + numLinksMatched + " route links. ";
		conditionalSpeeds = new Object();
		variableSpeeds = new Object();
		speed = new Object();
	}
