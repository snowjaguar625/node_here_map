...
var ROAD_GEOM_FC=[], LINK_ATTRIBUTE_FC=[], LINK_ATTRIBUTE2_FC=[], SPEED_LIMITS_FC=[], 
  ...
  ROAD_ADMIN_NAMES_FC=[], ROAD_ROUGHNESS_FC=[], TOPO_SEG_ID_FC=[], TURN_RESTR_FC=[], TOPO_SEG_LINK_FC = [], CELL_SIGNALS_FC=[];
...
function getServiceConfigLimit(){
  ...
  // Generate URL.
  // https://fleet.ls.hereapi.com/1/serviceconfiguration.json?apikey=
  var url = ['https://fleet.ls.hereapi.com/', 'serviceconfiguration.json?',
      '&apikey=',
      api_key_jp,
...
function processServiceConfigResponse(resp){
  ...
  else if(resp.responseCode && resp.responseCode.startsWith("4")){
...
function createTapLinkHandler(polyline) {
  return function (e) {
      ...
      else if(layerSelected == 'CELL_SIGNALS_FC'){infoText = CELL_SIGNALS_FC[linkId];}
      if (infoText == null) {
...
function getIndex(newIds, isTopologySegment){
  ...
  // https://fleet.ls.hereapi.com/1/index.json?layer=ROAD_GEOM_FCn&attributes=LINK_ID&values=52298144,52298166&apikey=
...
function getLinkGeometry(){
  ...
  // Generate URL.
  // https://fleet.ls.hereapi.com/1/index.json?layer=ROAD_GEOM_FC&attributes=LINK_ID&values=52298144,52298166&apikey=
  ...
  if (resp == null || resp.Tiles == null || resp.Tiles[0].Rows == null) {
      ...
      for (var t=0; t<resp.Tiles.length; t++){
          for (var r = 0; r < resp.Tiles[t].Rows.length; r++) {
              ...
              else if(layerSelected == 'CELL_SIGNALS_FC'){CELL_SIGNALS_FC[linkId]=resp.Tiles[t].Rows[r];}
...
function getPdeLinkInfo() {
  ...
  if (currStabTopoChecked !== stabTopoChecked) { // Stable Topology info was requested/not requested before. So trigger new PDE requests (not load from cache) 
      ...
      for (var l=0; l<linksInput.length; l++) {
          ...
          else if(layerSelected == 'CELL_SIGNALS_FC'){if (CELL_SIGNALS_FC[linkId]==null) {linksTobeRequested.push(linkId);}}
