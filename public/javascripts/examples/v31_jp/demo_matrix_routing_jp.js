/*
	...
	*/
    ...
    var startWps = [
        "35.43647396282902,139.55475245894866",
        "35.45330385718398,139.51031221381936",
        "35.42225485839007,139.50001253187503",
        "35.44053833858412,139.39214701542213",
        "35.37030023884116,139.46046823898618",
        "35.33198916596607,139.53588534249036",
        "35.3002703694319,139.59020870196676",
        "35.3281452337006,139.5630862061801"
	];
	var destinationWps = [
        "35.43647396282902,139.55475245894866",
        "35.45330385718398,139.51031221381936",
  ...
  var marker = new H.map.DomMarker(point);			
	...
	var calculateRouteDirect = function(start, destination)
	{
		console.log(destination);
			...
			var urlRoutingReq = "https://router.hereapi.com/v8/routes?apiKey=cF1MCUW-j-ThP4xHja2a7Y_x_Bq6tg6Nz_2CFSR4IwI&origin=" + start.lat + "," + start.lng +"&destination="+destination.lat+","+ destination.lng +"&routingMode=fast&transportMode=car&return=polyline,actions,instructions,summary,passthrough,routeHandle"
		...
		$.ajax({
				...
				var
					...
					shape = H.util.flexiblePolyline.decode(route.sections[0].polyline).polyline.flat(),
					i,
					l = shape.length;
           ...
           group.setZIndex(10);
			...
			group.addObject(polyline);
				// Update this place for MappedPosition equivalent entity of v7.2 Api.
				/*var point = {
  ...
  function onMatrixCalculated(r){
	...
	tableData = [];
		  ...
		  for(var i=0,lR=response.length; i<lR; i++){
			  ...
			  if(s !== ""){
				...
				calculateSimpleRoute(currStart, destinationWps[item.destinationIndex]);
			  }
		  }
	...
	function calculateSimpleRoute(wp0, wp1)
	{
		var urlRoutingReq = 
					...
					"?",
					  [
						["apiKey",api_key_jp].join("="),
						["origin",wp0].join("="),
						["destination",wp1].join("="),
						["routingMode","fast"].join("="),
						["transportMode","car"].join("="),
